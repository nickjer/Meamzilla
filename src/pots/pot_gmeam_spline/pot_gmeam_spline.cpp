...
#include "pot_gmeam_spline.h"
#include "../pot_meam_spline/atom_meam_spline.h"
#include "../pot_meam_spline/pair_meam_spline.h"
#include "../pot_meam_spline/triplet_meam_spline.h"
#include "../../input.h"
#include "../../config.h"
#include "../../cell.h"
#include "../../atom_vec.h"
...
PotGMEAMSpline::PotGMEAMSpline(class Meamzilla *mmz, int ntypes) : PotGMEAM(mmz, ntypes)
{
  // ctor
}
...
PotGMEAMSpline* PotGMEAMSpline::clone() const
{
  return new PotGMEAMSpline(*this);
}
...
double PotGMEAMSpline::compute(const Comm& comm, ErrorVec *error_vec)
{
...
void PotGMEAMSpline::compute_densities(const Comm& comm, Vector<double> *density_ptr)
{
  String common_basis_type = pot_fns_[0].get_common_basis_type();
...
template <class T>
double PotGMEAMSpline::fast_compute(const Comm& comm, ErrorVec *error_vec)
{
  ...
  for (Atom*& atom_i_ptr : mmz->atomvec->atoms) {
    ...
    AtomMEAMSpline &atom_i = *(static_cast<AtomMEAMSpline *>(atom_i_ptr));
    ...
    for (Pair*& pair_ij_ptr : atom_i.pairs) {
      PairMEAMSpline &pair_ij = *(static_cast<PairMEAMSpline *>(pair_ij_ptr));  // tmp pair

      // Check that neighbor length lies in pair potential radius
      if (pair_ij.phi_knot != -1) {
        AtomMEAMSpline &atom_j = *(static_cast<AtomMEAMSpline *>(pair_ij.neigh));  // tmp atom
    ...
    for (Triplet*& triplet_ijk_ptr : atom_i.triplets) {
      TripletMEAMSpline &triplet_ijk = *(static_cast<TripletMEAMSpline *>(triplet_ijk_ptr));  // tmp triplet
      PairMEAMSpline &pair_ij = *(static_cast<PairMEAMSpline *>(triplet_ijk.pair_ij));  // tmp pairs
      PairMEAMSpline &pair_ik = *(static_cast<PairMEAMSpline *>(triplet_ijk.pair_ik));
    ...
    for (Triplet*& triplet_ijk_ptr : atom_i.triplets) {
      ...
      PairMEAMSpline &pair_ij = *(static_cast<PairMEAMSpline *>(triplet_ijk.pair_ij));  // tmp pairs
      PairMEAMSpline &pair_ik = *(static_cast<PairMEAMSpline *>(triplet_ijk.pair_ik));
      AtomMEAMSpline &atom_j = *(static_cast<AtomMEAMSpline *>(pair_ij.neigh));  // tmp atoms
      AtomMEAMSpline &atom_k = *(static_cast<AtomMEAMSpline *>(pair_ik.neigh));
...
template <class T>
void PotGMEAMSpline::fast_compute_densities(const Comm& comm, Vector<double> *density_ptr)
{
  // Untrap procs if necessary
  if (is_trapped_) {
    int flag = 4;
  ...
  // Loop over all atoms in atomvec
  for (Atom*& atom_i_ptr : mmz->atomvec->atoms) {
    AtomMEAMSpline &atom_i = *(static_cast<AtomMEAMSpline *>(atom_i_ptr));  // tmp atom
