...
PotJMEAM::PotJMEAM(class Meamzilla *mmz, int ntypes) : PotMEAM(mmz, ntypes)
{
  // Double count pairs
  dbl_cnt_pair_ = 1;
  ...
  pot_fns_[5].setup_pot_fns(ntypes, PotFns::AlloyType::Pair_ij, PotFns::FnType::Radial);
  ...
  pot_fns_[6].setup_pot_fns(ntypes, PotFns::AlloyType::Triplet_i, PotFns::FnType::Undefined);
}
...
PotJMEAM* PotJMEAM::clone() const
{
  return new PotJMEAM(*this);
}
...
void PotJMEAM::check_if_similar(Potential *pot_ptr) const
{
  PotJMEAM& pot = *static_cast<PotJMEAM *>(pot_ptr);
...
double PotJMEAM::compute(const Comm& comm, ErrorVec *error_vec)
{
  ...
  for (Atom*& atom_i_ptr : mmz->atomvec->atoms) {
    ...
    AtomJMEAM &atom_i = *(static_cast<AtomJMEAM *>(atom_i_ptr));
    ...
    for (Pair*& pair_ij_ptr : atom_i.pairs) {
      PairJMEAM &pair_ij = *(static_cast<PairJMEAM *>(pair_ij_ptr));  // tmp pair

      // Check that neighbor length lies in pair potential radius
      if (pair_ij.phi_knot != -1) {
        AtomJMEAM &atom_j = *(static_cast<AtomJMEAM *>(pair_ij.neigh));  // tmp atom
    ...
    for (Triplet*& triplet_ijk_ptr : atom_i.triplets) {
      TripletJMEAM &triplet_ijk = *(static_cast<TripletJMEAM *>(triplet_ijk_ptr));  // tmp triplet
      PairJMEAM &pair_ij = *(static_cast<PairJMEAM *>(triplet_ijk.pair_ij));  // tmp pairs
      PairJMEAM &pair_ik = *(static_cast<PairJMEAM *>(triplet_ijk.pair_ik));
    ...
    for (Triplet*& triplet_ijk_ptr : atom_i.triplets) {
      ...
      PairJMEAM &pair_ij = *(static_cast<PairJMEAM *>(triplet_ijk.pair_ij));  // tmp pairs
      PairJMEAM &pair_ik = *(static_cast<PairJMEAM *>(triplet_ijk.pair_ik));
      AtomJMEAM &atom_j = *(static_cast<AtomJMEAM *>(pair_ij.neigh));  // tmp atoms
      AtomJMEAM &atom_k = *(static_cast<AtomJMEAM *>(pair_ik.neigh));
...
void PotJMEAM::write_punish(const ErrorVec& e_vec, String filename, int num) const
{
  filename += ".punish";
...
int PotJMEAM::rescale_3body(const Comm& comm, std::ostream *out, int flag)
{
  if (!flag) return 0; // Don't run rescaling if flag is 0

  PotMEAM::rescale_3body(comm, out, flag); // rescale MEAM piece first (always returns 1, unless flag=0)

  // Get scaling factor
  double max_p_mag = pot_fns_[5].get_max_y_mag();
  double b = 1.0/max_p_mag;

  // Scale beta first: f*f*g = (b*f)(b*f)(g/b^2)
  // Scale f-pot first
  std::vector<Basis *> p_fns = pot_fns_[5].fns;
  for (Basis *p_fn : p_fns)
    *p_fn *= b;

  // We scale g by 1/b^2
  std::vector<Basis *> q_fns = pot_fns_[6].fns;
  for (Basis *q_fn : q_fns)
    *q_fn /= b*b;

  // Output scaling factor to screen
  if (comm.is_root() && out)
    *out << "JMEAM potential scaling factor " << std::fixed << b << std::endl;
