...
PotMEAMSpline::PotMEAMSpline(class Meamzilla *mmz, int ntypes) : PotMEAM(mmz, ntypes)
{
  // ctor
}
...
PotMEAMSpline* PotMEAMSpline::clone() const
{
  return new PotMEAMSpline(*this);
}
...
double PotMEAMSpline::compute(const Comm& comm, ErrorVec *error_vec)
{
...
void PotMEAMSpline::compute_densities(const Comm& comm, Vector<double> *density_ptr)
{
  String common_basis_type = pot_fns_[0].get_common_basis_type();
...
template <class T>
double PotMEAMSpline::fast_compute(const Comm& comm, ErrorVec *error_vec)
{
...
template <class T>
void PotMEAMSpline::fast_compute_densities(const Comm& comm, Vector<double> *density_ptr)
{
  // Untrap procs if necessary
  if (is_trapped_) {
    int flag = 4;
